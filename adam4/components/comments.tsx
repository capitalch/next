...
function Comments({ pageComments, slug }:{pageComments: string[], slug:string}) {
    ...
    function setForm(arrayItem:any) {
        ...
        const arrIndex = arr.findIndex((value:any) => value.id === arrayItem.id)
        index = arrIndex + 1
        setIndex(index)
...
function getCommentsArray(pageComm:string[]) {
    const outArray: any[] = []
...
function SubmitForm({ startPos, props }:any) {
